<!-- 项目记忆索引。每条对应同目录下一个 .md 文件 -->

- [编码通用纪律](feedback_coding.md) — Config 保护 / 编辑前查调用方 / Search-first / Checkpoint / 服务重启
- [二文档机制](feedback_docs.md) — ISSUES + CLAUDE.md 分工；会话恢复靠项目记忆 + git log（COMMITS.md / SESSION-LOG.md 已删除）
- [团队启动配置](feedback_team_setup.md) — 默认 4 人 architect=Opus 其余 Sonnet / 成员结构 / persona 文件 / 生命周期
- [团队运转规则](feedback_team_workflow.md) — team-lead 铁律 / 派活纪律 / 成员约束 / Bug 7 步 / 功能 6 步
- [踩坑教训](feedback_lessons.md) — 跳过 review 自审 / 成员越权 commit / 批量改动回退 / 粗略方案驳回 / CDN 国内不通
- [git commit 只由 team-lead 执行](feedback_git_commit_team_lead_only.md) — review/方案完成后不要建议 commit
- [用户工作偏好](feedback_user_style.md) — 一步一步确认 / 先看方案再实施 / 不批量改 / 方案要逐帧详细 / 外部依赖本地化
- [项目开发进展](project_progress.md) — 会话 27：AgentDock 重构已 revert 回退到悬浮窗稳定版（5294446，本地未推送领先远端 1 提交）；实测全链路通过
- [按用户步骤走](feedback_follow_user_steps.md) — team-lead 不自行规划顺序，等用户指示再派活
- [发现问题及时修](feedback_fix_issues_immediately.md) — 不要因为"不阻塞"就搁置，当场修掉
- [前后端端口约定](reference_ports.md) — 后端 8080 / 前端 3081（3080 有缓存污染已弃用）
- [启动服务先查部署文档](feedback_deploy_first.md) — 不可凭印象启动，必须先读 DEPLOY.md
