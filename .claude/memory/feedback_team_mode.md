...
description: CC Team 启动方式 / 默认 4 人结构（不含 reviewer） / team-lead 6 铁律 / Bug 修复 7 步 / 功能增强 6 步 — 目前团队未启动，仅用户显式要求时启用
type: feedback
---
...
用户说"创建团队"等指令时，使用 **TeamCreate → Agent(team_name=...)** 一次性创建 **4 个成员**（PM / architect / frontend / backend）。**不是**在主对话中读 persona 文件然后角色扮演。

**默认不启动 reviewer**：reviewer 在日常开发中使用频率低、启动浪费资源。需要时由用户显式要求再追加创建。

**Why**：读 persona 只是信息加载，不等于启动团队协作模式。角色扮演一人分饰多角会混淆上下文、无法并行、拿不到 Team 工具链的分发能力。

## 成员模型默认 Sonnet

4 个成员（PM / architect / frontend / backend）默认 `model: "sonnet"`；team-lead（主对话）保持当前模型（通常 opus）做协调。
...
| architect | 架构方案、技术方向、设计评审、代码 review | 只给方向不给代码示例，不直接改代码 |
| frontend | 前端代码开发 | 不写文档、不做审查 |
| backend | 后端代码开发 | 不写前端、不写文档 |

> **reviewer（按需追加）**：代码审查、验证构建、测试、部署。日常开发不启动，用户显式要求时再创建。reviewer 不在时，代码 review 职责由 architect 承担。
...
- **创建/恢复**：TeamCreate → 一次性建 4 成员（PM/architect/frontend/backend） → team-lead 进入 leader 模式 → SendMessage 派活 → 成员 idle 待命 → 下个任务 SendMessage 唤醒（**不销毁不重建**）
...
- **Persona 文件**：`docs/team-personas/` 下有 5 个文件（pm.md / architect.md / backend-dev.md / frontend-dev.md / reviewer.md），默认只加载前 4 个，reviewer.md 按需加载

## 血的教训

**架构师出方案后，team-lead 不能自己动手写代码。** 必须把方案通过 SendMessage 派给 frontend/backend 成员执行。即使 team-lead 有能力做、即使"顺手就改了"，也不能越权。用户要求的是团队协作流程，不是结果。流程本身就是产出的一部分。

**Why**：2026-05-09 事件 — 三个 agent（PM/architect/frontend）完成研究后，team-lead 自己把 800 行代码改动全做了，完全绕过了前端开发 agent。用户明确批评"所有工作都是你自己完成的"。
