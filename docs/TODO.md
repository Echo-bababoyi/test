...
- 🔲 **Phase 1 终验收**（所有 11 页面灰块线框完成，下一步确认主线端到端通、准备 Phase 2）← **下一步**
...
- 🔲 在 `CLAUDE.md` 补导航约定：`go` = 根级/鉴权切换（如 /elder ↔ /home）；**`push` = 子级钻取（搜索→结果、结果→服务详情、认证流中间态都应 push）**。批次 4 架构师原判断"服务详情归终结用 go"在批次 5 验收时被用户发现是错的，已 hotfix 改 push
- 🔲 `MyPage` FAB 颜色 `Colors.grey[700]` —— 截图里像灰，但 reviewer 批次 5 建议改 `elderPrimary`，Phase 2 上真机再定
- 🔲 `SearchResultPage` 编辑态切换时 URL 现保持 stale（点 × 改文字再提交后地址栏仍是旧 query）——Phase 2 评估是否改 `context.replace` 同步
...
### 2026-04-18 · `<pending>` · feat(phase-1): 批次 5 — SocialInsurance + PensionQuery + MyPage（含两 hotfix，Phase 1 结构收尾）
