...
**Tab 区**
- [ ] 顶部 TabBar 3 个 Tab：热门服务 / 我的常用 / 我的订阅
- [ ] Tab 切换流畅，各 Tab 内容独立（不共用滚动位置）
- [ ] 默认选中「热门服务」

**热门服务 Tab 滚动体（3 段拼接）**
- [ ] 段一：顶部搜索栏 + Banner 轮播（或占位） + 第一批服务入口网格
- [ ] 段二：第二批服务入口网格（与段一无重叠）
- [ ] 段三：底部功能区块 + 页脚（与段二无重叠）
- [ ] 3 段拼接为单一 `CustomScrollView`/`SingleChildScrollView`，无分页断点

**我的常用 / 我的订阅 Tab**
- [ ] 各有对应占位卡片，与截图区块规划一致

**弹窗**
- [ ] 未登录进入时：立即登录 **InAppOverlay** 自动弹出（非阻塞底部浮层）
- [ ] 弹窗可手势关闭或点「×」关闭
- [ ] 已登录进入时：不弹窗

**底部导航**
- [ ] 「首页」与「我的」两项可切换，与 `/my` 联通
- [ ] 主色 `#FF6D00`（橙色）
...
| # | 弹窗名称 | 触发页面 | 弹窗类别 | 阻塞？ | 对应截图 |
|---|---------|---------|---------|-------|---------|
| 1 | 立即登录提示 | `ElderHomePage` | `InAppOverlay` | 否 | `长辈版主页面-立即登录弹窗.jpg` |
| 2 | 同意条款 | `LoginPage` | `InAppOverlay` | 否 | `登录页面-同意条款弹窗.jpg` |
| 3 | 请求刷脸认证 | `FaceAuthPage` | `InAppOverlay` | 否 | `刷脸身份验证页面-请求刷脸认证弹窗.jpg` |
| 4 | 摄像头系统权限 | `FaceAuthPage` | `SystemDialog` | 是 | `刷脸身份验证页面-请求使用摄像头弹窗.jpg` |
| 5 | 验证码确认 | `VerifyPage` | `SystemDialog` | 是 | `验证码页面-验证码弹窗.jpg` |
| 6 | 麦克风权限引导 | `SearchPage` | `InAppOverlay` | 否 | `搜索页面-获取麦克风权限.jpg` |
| 7 | 麦克风系统权限 | `SearchPage` | `SystemDialog` | 是 | `搜索页面-获取麦克风权限弹窗.jpg` |
| 8 | 麦克风录音浮层 | `SearchPage` | `InAppOverlay` | 否 | `搜索页面-麦克风说话按钮弹窗.jpg` |
| 9 | 其他认证方式选择 | `FaceAuthPage` | `InAppOverlay`（推测） | 否 | 无（截图未收录） |

**注：** 弹窗 9 无对应截图，页面逻辑文档仅提及「其他认证方式弹窗」，Phase 2 时需根据原版 App 观察补全。两类弹窗（`SystemDialog` 3 个 / `InAppOverlay` 6 个）在代码中必须是**两套独立组件族**，不共用基类。
